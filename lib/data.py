...
def load_data(data_path, img_ratio=8):
    def img_transform(img, img_ratio=img_ratio):
    ...
    img_rgb = tf.stack(img_rgb, axis=0)
    ...
    data = hwf_rgb_to_data(hwf, img_rgb)

    return data

def hwf_rgb_to_data(hwf, rgb):
    cam_rays = get_local_rays(int(hwf[0]), int(hwf[1]), hwf[2])
    f_cam_rays = tf.reshape(cam_rays, [-1, 3])
    
    data = {'hwf': hwf, 'img_rgb': rgb, 'f_cam_rays': f_cam_rays}
